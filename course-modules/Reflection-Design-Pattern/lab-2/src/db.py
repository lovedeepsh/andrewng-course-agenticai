"""SQLite helpers for the SQL-reflection lab: build the DB, read schema, run queries."""
...
def create_transactions_db(
    ...
    """Create an SQLite DB with a single event-sourced 'transactions' table."""
    ...
    conn.commit()
    conn.close()
    print(f"SQLite database '{db_path}' created with a single 'transactions' table.")


def get_schema(db_path: str) -> str:
    """Return the schema string for the 'transactions' table."""
    conn = sqlite3.connect(db_path)
    cur = conn.cursor()
...
def execute_sql(query: str, db_path: str) -> pd.DataFrame:
    """Run a SELECT over 'transactions'. On error, return a one-row DataFrame with an 'error' column."""
